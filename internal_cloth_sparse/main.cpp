...
const int ndiv = 25;  // (in) number of division of the square cloth edge, 一辺の分割数
...
const double stiff_bend = 1.0e-3; // bending stiffness 曲げ剛性
...
double time_step_size = 0.02; // size of time step，時間ステップの大きさ
...
// 疎行列ソルバのための変数
CMatrixSquareSparse mat_A; // 係数行列クラス
CPreconditionerILU  ilu_A; // 係数行列をILU分解したデータを格納するクラス

std::vector<double> aNormal; // deformed vertex noamals，変形中の頂点の法線(可視化用)

// data for camera,カメラ制御のための変数群
bool is_animation;
double camera_scale = 1.0;
double camera_qrot[4] = {1,0,0,0};
double camera_trans[3] = {0,0,0.5};
...
void StepTime()
{
  ...
  /*
   // solving lienar system using conjugate gradient method
   StepTime_InternalDynamics(aXYZ, aUVW, mat_A,
   aXYZ0, aBCFlag,
   aTri, aQuad,
   time_step_size,
...
void myGlutMotion( int x, int y ){
  ...
  if( ibutton == GLUT_LEFT_BUTTON && imodifier == GLUT_ACTIVE_CTRL ){ // rotate view
  ...
  if( ibutton == GLUT_LEFT_BUTTON && imodifier == GLUT_ACTIVE_SHIFT ){ // translate view
    double v[3] = {
...
int main(int argc,char* argv[])
{
  { // initialze data
    ...
    mat_A.Initialize(np,3); // 疎行列のサイズを指定
    CJaggedArray crs;
    crs.SetEdgeOfElem(aQuad, (int)aQuad.size()/4, 4, np, false);
    crs.Sort();
    mat_A.SetPattern(crs.index, crs.array);  // 係数行列の非ゼロパターンを指定
    ilu_A.Initialize_ILU0(mat_A); // ILU前処理行列に，係数行列の非ゼロパターンを設定
  }
